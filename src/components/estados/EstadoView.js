...
import { crearEstadoEquipo, editarEstadoEquipo, getEstadoEquipo } from '../../services/estadoEquipoService';
...
export const EstadoView = () => {
    ...
    const handleOnSubmit = async (e) => {
        ...
        try {
            ...
            if (valoresForm.id) {
                await editarEstadoEquipo(valoresForm)

            } else {
                await crearEstadoEquipo(estado);
            }
        ...
        <div className="container-fluid mt-3 mb-2 p-3">
            <div className="card">
                <div className='card-header'>
                    <h5 className='card-title'>Estados de Equipos</h5>
                </div>
                    ...
                    <Table dataTableHead={dataTableHead}>
                        {estados.map((estado) => (
                            <tr key={estado._id}>
                                ...
                                <td>{estado.nombre}</td>
                                <td>{estado.estado}</td>
                                <td>{moment(estado.fechaCreacion).format('MMMM Do YYYY, h:mm:ss')}</td>
                                <td>{moment(estado.fechaActualizacion).format('MMMM Do YYYY, h:mm:ss')}</td>
                                <td>
                                    <button className="btn btn-primary" onClick={(e) => handleUpdate(e, estado)}>
                                        Editar
                                    </button>
                                </td>
                            </tr>
